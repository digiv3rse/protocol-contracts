/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import type {
  BaseContract,
  BigNumber,
  BytesLike,
  CallOverrides,
  ContractTransaction,
  Overrides,
  PopulatedTransaction,
  Signer,
  utils,
} from "ethers";
import type { FunctionFragment, Result } from "@ethersproject/abi";
import type { Listener, Provider } from "@ethersproject/providers";
import type {
  TypedEventFilter,
  TypedEvent,
  TypedListener,
  OnEvent,
} from "../common";

export interface MockContractInterface extends utils.Interface {
  functions: {
    "initialize(address)": FunctionFragment;
    "testMockContract()": FunctionFragment;
  };

  getFunction(
    nameOrSignatureOrTopic: "initialize" | "testMockContract"
  ): FunctionFragment;

  encodeFunctionData(functionFragment: "initialize", values: [string]): string;
  encodeFunctionData(
    functionFragment: "testMockContract",
    values?: undefined
  ): string;

  decodeFunctionResult(functionFragment: "initialize", data: BytesLike): Result;
  decodeFunctionResult(
    functionFragment: "testMockContract",
    data: BytesLike
  ): Result;

  events: {};
}

export interface MockContract extends BaseContract {
  connect(signerOrProvider: Signer | Provider | string): this;
  attach(addressOrName: string): this;
  deployed(): Promise<this>;

  interface: MockContractInterface;

  queryFilter<TEvent extends TypedEvent>(
    event: TypedEventFilter<TEvent>,
    fromBlockOrBlockhash?: string | number | undefined,
    toBlock?: string | number | undefined
  ): Promise<Array<TEvent>>;

  listeners<TEvent extends TypedEvent>(
    eventFilter?: TypedEventFilter<TEvent>
  ): Array<TypedListener<TEvent>>;
  listeners(eventName?: string): Array<Listener>;
  removeAllListeners<TEvent extends TypedEvent>(
    eventFilter: TypedEventFilter<TEvent>
  ): this;
  removeAllListeners(eventName?: string): this;
  off: OnEvent<this>;
  on: OnEvent<this>;
  once: OnEvent<this>;
  removeListener: OnEvent<this>;

  functions: {
    initialize(
      initializationAddress: string,
      overrides?: CallOverrides
    ): Promise<[void]>;

    testMockContract(
      overrides?: Overrides & { from?: string }
    ): Promise<ContractTransaction>;
  };

  initialize(
    initializationAddress: string,
    overrides?: CallOverrides
  ): Promise<void>;

  testMockContract(
    overrides?: Overrides & { from?: string }
  ): Promise<ContractTransaction>;

  callStatic: {
    initialize(
      initializationAddress: string,
      overrides?: CallOverrides
    ): Promise<void>;

    testMockContract(overrides?: CallOverrides): Promise<void>;
  };

  filters: {};

  estimateGas: {
    initialize(
      initializationAddress: string,
      overrides?: CallOverrides
    ): Promise<BigNumber>;

    testMockContract(
      overrides?: Overrides & { from?: string }
    ): Promise<BigNumber>;
  };

  populateTransaction: {
    initialize(
      initializationAddress: string,
      overrides?: CallOverrides
    ): Promise<PopulatedTransaction>;

    testMockContract(
      overrides?: Overrides & { from?: string }
    ): Promise<PopulatedTransaction>;
  };
}
