...
if [[ $1 == "" ]]
    then
        ...
        echo "    e.g. contractName (optional): DiGiHubInitializable (default)"
        echo "    e.g. proxyNameInAddresses (optional): DiGiHubProxy (default)"
        echo "    e.g. newImplNameInAddresses (optional): DiGiHubV2Impl (default)"
        echo ""
        echo "Example:"
        echo "    verifyStorageSlots.sh mainnet DiGiHub DiGiHubProxy DiGiHubV2Impl"
        exit 1
fi
...
if [[ $2 == "" ]]
    then
        CONTRACT_NAME="DiGiHubInitializable"
    else
        CONTRACT_NAME=$2
fi
