...
const addressesJson = require(join(__dirname, addressesPath));
...
writeFileSync(`./backendEnv_${targetEnv}.txt`, '');

// And then write to backendEnv.txt line by line
const str = createWriteStream(`./backendEnv_${targetEnv}.txt`, { flags: 'a' });

str.write(`# Hub\n`);
str.write(`DIGI_HUB_PROXY=${addresses['DiGiHubProxy']}\n`);

str.write(`\n# DiGiHandles\n`);
str.write(`DIGI_HANDLE_PROXY=${addresses['DiGiHandles']}\n`);

str.write(`\n# TokenHandleRegistry\n`);
str.write(`DIGI_TOKEN_HANDLE_REGISTRY_PROXY=${addresses['TokenHandleRegistry']}\n`);

str.write(`\n# Public Act Proxy\n`);
str.write(`DIGI_PUBLIC_ACT_PROXY=${addresses['PublicActProxy']}\n`);
...
str.write(`LEGACY_PERIPHERY_DATA_PROVIDER=${addresses['DiGiPeriphery']}\n`);
...
str.write(
  `DIGI_COLLECT_PUBLICATION_ACTION_PROXY=${findModule('act', 'CollectPublicationAction', 'v2')}\n`
);
