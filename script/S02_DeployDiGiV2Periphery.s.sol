...
contract S02_DeployDiGiV2Periphery is Script, ForkManagement, ArrayHelpers {
    // add this to be excluded from coverage report
    function testDiGiV2DeployPeriphery() public {}
    ...
    DiGiAccount deployer;
    DiGiAccount governance;
    ...
    function loadBaseAddresses() internal override {
        ...
        hub = IDiGiHub(json.readAddress(string(abi.encodePacked('.', targetEnv, '.DiGiHubProxy'))));
        vm.label(address(hub), 'DiGiHub');
        console.log('DiGi Hub Proxy: %s', address(hub));

        handles = IDiGiHandles(json.readAddress(string(abi.encodePacked('.', targetEnv, '.DiGiHandles'))));
        vm.label(address(handles), 'DiGiHandles');
        console.log('DiGi Handles: %s', address(handles));
    ...
    function _writeBackendEnvFile() internal {
        ...
        vm.writeLine(backendEnv, string.concat('DIGI_HUB_PROXY=', vm.toString(address(hub))));
        vm.writeLine(backendEnv, '## DiGiHandles');
        vm.writeLine(backendEnv, string.concat('DIGI_HANDLE_PROXY=', vm.toString(address(handles))));
        vm.writeLine(backendEnv, '# TokenHandleRegistry');
        vm.writeLine(
            backendEnv,
            string.concat('DIGI_TOKEN_HANDLE_REGISTRY_PROXY=', vm.toString(address(tokenHandleRegistry)))
        );
        vm.writeLine(backendEnv, '# Collection open actions');
        vm.writeLine(
            backendEnv,
            string.concat('DIGI_COLLECT_PUBLICATION_ACTION_PROXY=', vm.toString(address(collectPublicationAction)))
        );
