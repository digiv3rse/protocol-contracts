...
contract FollowNFTTest is BaseTest, DiGiBaseERC721Test {
    ...
    function _mintERC721(address to) internal virtual override returns (uint256) {
        vm.assume(!_isDiGiHubProxyAdmin(to));
    ...
    function testGetTokenURI() public {
        ...
        assertEq(
            decodedTokenURI.readString('.description'),
            string.concat('DiGi Protocol - Follower #', tokenIdAsString, ' of Profile #', vm.toString(targetProfileId))
        );
        assertEq(decodedTokenURI.readUint('.attributes[0].value'), tokenId, "Token ID doesn't match");
        assertEq(
            decodedTokenURI.readUint('.attributes[1].value'),
    ...
    // No negatives when minting a new token, all the failing cases will occur at DiGiHub level. See `FollowTest.t.sol`.
