...
contract ProxyAdminTest is BaseTest {
    ...
    function testCannot_changeDiGiHubProxyAdmin_ifNotOwner(address newAdmin, address otherAddress) public {
        vm.assume(otherAddress != proxyAdminContractOwner);
    ...
    function testContructor() public notFork {
        assertEq(address(proxyAdminContract.DIGI_HUB_PROXY()), address(hub), 'Hub address is not set correctly');
        assertEq(
    ...
    function testChangeDiGiHubProxyAdmin(address newAdmin) public {
        vm.assume(newAdmin != address(0));
