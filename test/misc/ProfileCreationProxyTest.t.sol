...
contract ProfileCreationProxyTest is BaseTest {
    ...
    function testProxyCreateHandle(address handleOwner) public {
        ...
        vm.expectCall(address(digiHandles), abi.encodeCall(digiHandles.mintHandle, (handleOwner, handle)));

        vm.prank(profileCreationProxyOwner);
        uint256 handleId = profileCreationProxy.proxyCreateHandle(handleOwner, handle);

        assertEq(digiHandles.ownerOf(handleId), handleOwner);
    }
    ...
    function testProxyCreateProfileWithHandle(address profileOwner) public {
        ...
        uint256 predictedHandleId = digiHandles.getTokenId(handle);
        ...
        vm.expectCall(
            address(digiHandles),
            abi.encodeCall(digiHandles.mintHandle, (address(profileCreationProxy), handle)),
            1
        );
        vm.expectCall(
            address(tokenHandleRegistry),
        ...
        assertEq(digiHandles.ownerOf(handleId), profileOwner, 'Handle owner mismatch');
