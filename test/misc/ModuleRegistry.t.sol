...
contract ModuleRegistryTest is BaseTest {
    bytes4 constant DIGI_MODULE_INTERFACE_ID = bytes4(keccak256(abi.encodePacked('DIGI_MODULE')));
    ...
    function testCannotRegisterModule_IfDoesNotSupportDiGiModuleInterface() public {
        address module = makeAddr('module');
        uint256 moduleType = uint256(IModuleRegistry.ModuleType.PUBLICATION_ACTION_MODULE);

        _mockSupportsInterface(module, DIGI_MODULE_INTERFACE_ID, false);

        vm.expectRevert(ModuleRegistry.NotDiGiModule.selector);
    ...
    function testCannotRegisterModule_IfModuleDoesNotSupportType() public {
        ...
        _mockSupportsInterface(module, DIGI_MODULE_INTERFACE_ID, true);
    ...
    function _mockSupportsInterface(address module, bytes4 interfaceId, bool isSupported) private {
        bytes memory data = abi.encodeCall(IDiGiModule.supportsInterface, (interfaceId));
    ...
    function _mockModuleMetadataURI(address module, string memory metadataURI) private {
        bytes memory data = abi.encodeCall(IDiGiModule.getModuleMetadataURI, ());
