...
contract ProfileNFTTest is DiGiBaseERC721Test, TokenGuardianTest_Default_On {
    ...
    function testGetTokenURI_Fuzz() public {
        ...
        for (uint256 profileId = type(uint256).max; profileId > 0; profileId >>= 2) {
            ...
            assertEq(
                ...
                string.concat('DiGi Protocol - Profile #', profileIdAsString)
            );
    ...
    function _assumeNotProxyAdmin(address account) internal view virtual override {
        vm.assume(!_isDiGiHubProxyAdmin(account));
    }
    ...
    function testCannotSetRoyaltiesIf_NotGovernance(
        ...
        vm.assume(!_isDiGiHubProxyAdmin(nonGovernanceAddress));
