...
contract MigrationsTestNonFork is BaseTest {
    ...
    function testCannotMigrateProfile_Twice() public notFork {
        ...
        _mockProfileDeprecatedHandle(profileId, 'profile.digi');
        ...
        bytes memory data = abi.encodeCall(DiGiHandles.migrateHandle, (profileOwner, 'profile'));

        vm.expectCall(address(digiHandles), 0, data, 0);
        hub.batchMigrateProfiles(profileIds);
    }
    ...
    function testProfileMigration() public notFork {
        _mockProfileDeprecatedHandle(1, 'digiprotocol');
        _mockProfileDeprecatedHandle(2, 'profile.digi');
        _mockProfileDeprecatedHandle(3, 'with-dash.digi');
        _mockProfileDeprecatedHandle(4, 'with_underscore.digi');
        _mockProfileDeprecatedHandle(5, 'l0ng3r_pr0fil3_with_numb3r.digi');
        ...
        expectedHandleIds[0] = uint256(keccak256(bytes('digiprotocol')));
        ...
        for (uint256 i = 0; i < 5; i++) {
            assertEq(profileOwners[i], digiHandles.ownerOf(expectedHandleIds[i]));
        }
    }
    ...
    function testMigrateFollowModules_IfProfileHasLegacyFeeFollowModule() public notFork {
        ...
        Types.MigrationParams memory migrationParams = Types.MigrationParams({
            digiHandlesAddress: makeAddr('digiHandlesAddress'),
        ...
        DiGiV2Migration migration = new DiGiV2Migration(migrationParams);
