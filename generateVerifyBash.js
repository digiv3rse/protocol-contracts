...
async function findDeploymentDirectory(basePath, prefix, chainId) {
  ...
  if (chainId) {
    return join(basePath, filtered[0], ChainId);
  } else {
    return join(basePath, filtered[0]);
  }
}
...
async function findApiDirectory(basePath, prefix) {
  ...
  for (const dir of filteredDirs) {
    const contents = await fs.readdir(join(basePath, dir));
  ...
  return join(basePath, foundDirs[0]);
}
...
async function encodeConstructorArgs(contractName, args) {
  ...
  const filePath = join(contractDirectory, `${contractName}.json`);
...
async function generateBashScript() {
  ...
  const fileContents = await fs.readFile(join(deploymentPath, targetFile), 'utf-8');
  const data = JSON.parse(fileContents);
