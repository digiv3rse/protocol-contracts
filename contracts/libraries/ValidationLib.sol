...
library ValidationLib {
    ...
    function _validateReferrerAsMirrorOrCommentOrQuote(
        ...
        if (_referrerPub.pointedProfileId != targetedProfileId || _referrerPub.pointedPubId != targetedPubId) {
            // ...or if the referrer pub's root is the target pub (i.e. target pub is a DiGi V2 post)...
            if (_referrerPub.rootProfileId != targetedProfileId || _referrerPub.rootPubId != targetedPubId) {
                ...
                if (
                    // Here the target pub must be a "pure" DiGi V2 comment/quote, which means there is no
                    // DiGi V1 Legacy comment or post on its tree of interactions, and its root pub is filled.
                    // Otherwise, two DiGi V2 "non-pure" publications could be passed as a referrer to each other,
