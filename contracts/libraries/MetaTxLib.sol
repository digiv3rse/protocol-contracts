...
library MetaTxLib {
    ...
    /**
     * @dev We store the domain separator and DiGiHub Proxy address as constants to save gas.
     *
     * keccak256(
     *     abi.encode(
    ...
    function calculateDomainSeparator() internal view returns (bytes32) {
        if (address(this) == DIGI_HUB_ADDRESS && block.chainid == POLYGON_CHAIN_ID) {
            return DIGI_HUB_CACHED_POLYGON_DOMAIN_SEPARATOR;
        }
        return
            keccak256(
                abi.encode(
                    Typehash.EIP712_DOMAIN,
                    keccak256(bytes(IDiGiERC721(address(this)).name())),
