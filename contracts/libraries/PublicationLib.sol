...
library PublicationLib {
    function MODULE_REGISTRY() internal view returns (IModuleRegistry) {
        return IModuleRegistry(IDiGiHub(address(this)).getModuleRegistry());
    }
    ...
    function _fillRootOfPublicationInStorage(
        ...
        if (pubPointedType == Types.PublicationType.Post) {
            // The publication pointed is a DiGi V2 post.
            ...
            // The publication pointed is either a DiGi V2 comment or a DiGi V2 quote.
            // Note that even when the publication pointed is a V2 one, it will lack `rootProfileId` and `rootPubId` if
            // there is a DiGi V1 Legacy publication in the thread of interactions (including the root post itself).
            _publication.rootPubId = _pubPointed.rootPubId;
            return _publication.rootProfileId = _pubPointed.rootProfileId;
        }
        // Otherwise the root is not filled, as the pointed publication is a DiGi V1 Legacy publication, which does not
        // support DiGi V2 referral system.
        return 0;
    }
