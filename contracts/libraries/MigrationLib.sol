...
library MigrationLib {
    ...
    function batchMigrateProfiles(
        ...
        while (i < profileIds.length) {
            _migrateProfile(profileIds[i], digiHandles, tokenHandleRegistry);
            unchecked {
                ++i;
            }
        }
    ...
    function _migrateProfile(
        ...
        if (profileOwner != address(0)) {
            ...
            // We check if the profile is the "digiprotocol" profile by checking profileId != 1.
            // "digiprotocol" is the only edge case without the .digi suffix:
            if (profileId != DIGI_PROTOCOL_PROFILE_ID) {
                assembly {
                    let handle_length := mload(handle)
                    mstore(handle, sub(handle_length, DOT_DIGI_SUFFIX_LENGTH)) // Cut 5 chars (.digi) from the end
                }
            }
            // We mint a new handle on the DiGiHandles contract. The resulting handle NFT is sent to the profile owner.
            uint256 handleId = digiHandles.migrateHandle(profileOwner, handle);
    ...
    function batchMigrateFollowModules(
        ...
        while (i < profileIds.length) {
            ...
            if (currentFollowModule == legacyFeeFollowModule) {
                ...
                // If the profile had `ProfileFollowModule` set, we just remove the follow module, as in DiGi V2
                // you can only follow with a DiGi profile.
