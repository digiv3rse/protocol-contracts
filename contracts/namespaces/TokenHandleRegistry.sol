...
contract TokenHandleRegistry is ITokenHandleRegistry {
    ...
    // First version of TokenHandleRegistry only works with DiGi Profiles and .digi namespace.
    ...
    constructor(address digiHub, address digiHandles) {
        DIGI_HUB = digiHub;
        DIGI_HANDLES = digiHandles;
    }

    // DiGi V1 to DiGi V2 migration function
    // WARNING: It is able to link the Token and Handle even if they're not in the same wallet.
    //          But it is designed to be only called from DiGiHub migration function, which assures that they are.
    function migrationLink(uint256 handleId, uint256 profileId) external {
        if (msg.sender != DIGI_HUB) {
            revert RegistryErrors.OnlyDiGiHub();
        }
        _executeLinkage(
            RegistryTypes.Handle({collection: DIGI_HANDLES, id: handleId}),
            RegistryTypes.Token({collection: DIGI_HUB, id: profileId}),
            address(0)
        );
    }
    ...
    function _link(uint256 handleId, uint256 profileId, address transactionExecutor) private {
        ...
        address profileOwner = IDiGiHub(DIGI_HUB).ownerOf(profileId);
        if (profileOwner != IDiGiHandles(DIGI_HANDLES).ownerOf(handleId)) {
            revert RegistryErrors.HandleAndTokenNotInSameWallet();
        }
        if (
            transactionExecutor != profileOwner &&
            !IDiGiHub(DIGI_HUB).isDelegatedExecutorApproved(profileId, transactionExecutor)
        ) {
    ...
    function _unlink(uint256 handleId, uint256 profileId, address transactionExecutor) private {
        ...
        if (
            IDiGiHandles(DIGI_HANDLES).exists(handleId) &&
            IDiGiHandles(DIGI_HANDLES).ownerOf(handleId) != transactionExecutor &&
            IDiGiHub(DIGI_HUB).exists(profileId) &&
            (IDiGiHub(DIGI_HUB).ownerOf(profileId) != transactionExecutor &&
                !IDiGiHub(DIGI_HUB).isDelegatedExecutorApproved(profileId, transactionExecutor))
        ) {
            revert RegistryErrors.NotHandleNorTokenOwner();
        }
        RegistryTypes.Handle memory handle = RegistryTypes.Handle({collection: DIGI_HANDLES, id: handleId});
    ...
    function resolve(uint256 handleId) external view returns (uint256) {
        if (!IDiGiHandles(DIGI_HANDLES).exists(handleId)) {
            revert RegistryErrors.DoesNotExist();
        }
        uint256 resolvedTokenId = _resolveHandleToToken(RegistryTypes.Handle({collection: DIGI_HANDLES, id: handleId}))
            .id;
        if (resolvedTokenId == 0 || !IDiGiHub(DIGI_HUB).exists(resolvedTokenId)) {
            return 0;
        }
    ...
    function getDefaultHandle(uint256 profileId) external view returns (uint256) {
        if (!IDiGiHub(DIGI_HUB).exists(profileId)) {
            revert RegistryErrors.DoesNotExist();
        }
        uint256 defaultHandleId = _resolveTokenToHandle(RegistryTypes.Token({collection: DIGI_HUB, id: profileId})).id;
        if (defaultHandleId == 0 || !IDiGiHandles(DIGI_HANDLES).exists(defaultHandleId)) {
            return 0;
        }
