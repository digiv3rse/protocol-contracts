 ...
 */
contract DiGiHandles is ERC721, ERC2981CollectionRoyalties, ImmutableOwnable, IDiGiHandles {
    using Address for address;

    // We used 31 to fit the handle in a single slot, with `.digi` that restricted localName to use 26 characters.
    ...
    modifier onlyOwnerOrWhitelistedProfileCreator() {
        if (msg.sender != OWNER && !IDiGiHub(DIGI_HUB).isProfileCreatorWhitelisted(msg.sender)) {
    ...
    ) ERC721('', '') ImmutableOwnable(owner, digiHub) {
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC2981CollectionRoyalties, ERC721, IERC165) returns (bool) {
    ...
    function _getReceiver(uint256 /* tokenId */) internal view override returns (address) {
        return IDiGiHub(DIGI_HUB).getTreasury();
    }
