 ...
 *
 * @notice This contract enables additional access control for encrypted publications on DiGi by reporting whether
...
contract LitAccessControl {
    ...
    function isFollowing(
        ...
        if (
            ...
            IDiGiHub(DIGI_HUB).isFollowing(followerProfileId, profileId) &&
    ...
    function hasCollected(
        ...
        // We get the collect NFT as if the publication is a DiGi V2 one
        address collectNFT = CollectPublicationAction(COLLECT_PUB_ACTION).getCollectData(publisherId, pubId).collectNFT;

        if (collectNFT == address(0)) {
            // If there is no collect NFT, we get the collect NFT of the publication as a DiGi V1 one
            collectNFT = IDiGiHub(DIGI_HUB).getPublication(publisherId, pubId).__DEPRECATED__collectNFT;
            if (collectNFT == address(0)) {
        ...
        return IERC721(collectNFT).balanceOf(IERC721(DIGI_HUB).ownerOf(collectorProfileId)) > 0;
    }
    ...
    function _isOwnerOrDelegatedExecutor(address requestorAddress, uint256 profileId) internal view returns (bool) {
        return
            requestorAddress == IERC721(DIGI_HUB).ownerOf(profileId) ||
            IDiGiHub(DIGI_HUB).isDelegatedExecutorApproved(profileId, requestorAddress);
    }
}
