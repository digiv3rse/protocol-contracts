...
/**
 * @custom:upgradeable Beacon proxy. The beacon, responsible for returning the implementation address, is the DiGiHub.
 */
contract FollowNFT is HubRestricted, DiGiBaseERC721, ERC2981CollectionRoyalties, IFollowNFT {
    ...
    modifier whenNotPaused() {
        if (IDiGiHub(HUB).getState() == Types.ProtocolState.Paused) {
    ...
    function initialize(uint256 profileId) external override {
        // This is called right after deployment by the DiGiHub, so we can skip the onlyHub check.
        if (_initialized) {
    ...
    function unfollow(uint256 unfollowerProfileId) external override onlyHub {
        ...
        address followTokenOwner = _unsafeOwnerOf(followTokenId);
        // DiGiHub already validated that this action can only be performed by the unfollower profile's owner or one of
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(DiGiBaseERC721, ERC2981CollectionRoyalties) returns (bool) {
        return
            DiGiBaseERC721.supportsInterface(interfaceId) || ERC2981CollectionRoyalties.supportsInterface(interfaceId);
    }
    ...
    function tokenURI(uint256 followTokenId) public view override returns (string memory) {
        ...
        return
            IFollowTokenURI(IDiGiHub(HUB).getFollowTokenURIContract()).getTokenURI(
                followTokenId,
                _followedProfileId,
    ...
    function _replaceFollower(
        ...
        if (currentFollowerProfileId != 0) {
            ...
            IDiGiHub(HUB).emitUnfollowedEvent(currentFollowerProfileId, _followedProfileId, transactionExecutor);
        } else {
            unchecked {
                _followerCount++;
            }
    ...
    function _unfollowIfHasFollower(uint256 followTokenId, address transactionExecutor) internal {
        ...
        if (followerProfileId != 0) {
            _unfollow(followerProfileId, followTokenId);
            IDiGiHub(HUB).emitUnfollowedEvent(followerProfileId, _followedProfileId, transactionExecutor);
        }
    }
    ...
    function _getReceiver(uint256 /* followTokenId */) internal view override returns (address) {
        if (!IDiGiHub(HUB).exists(_followedProfileId)) {
            return address(0);
        }
