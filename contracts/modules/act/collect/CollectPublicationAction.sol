 ...
 */
contract CollectPublicationAction is DiGiModuleMetadataInitializable, HubRestricted, IPublicationActionModule {
    ...
    function registerCollectModule(address collectModule) public returns (bool) {
        if (_collectModuleRegistered[collectModule]) {
            ...
            if (!IDiGiModule(collectModule).supportsInterface(type(ICollectModule).interfaceId)) {
                revert NotCollectModule();
            }

            string memory metadata = IDiGiModule(collectModule).getModuleMetadataURI();
