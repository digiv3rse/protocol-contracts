...
contract DiGiSeaDropCollection is ERC721SeaDropCloneable {
    ...
    function updatePublicDrop(address seaDropImpl, PublicDrop calldata publicDrop) external virtual override {
        // We only enforce the fees to cover the DiGi Treasury fees when using the default SeaDrop, as it is the SeaDrop
        // chosen by DiGi.
        if (seaDropImpl == DEFAULT_SEADROP && publicDrop.feeBps < IDiGiHub(HUB).getTreasuryFee()) {
            revert FeesDoNotCoverDiGiTreasury();
        }
